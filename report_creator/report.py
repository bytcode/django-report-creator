...
from report_creator.exporters.excel import ExcelExporter
from report_creator.exporters.pdf import PdfExporter
from report_creator.forms import ConfigForm, GroupByForm, FilterForm
from report_creator.utils import base_label, ReportValue, ReportRow
from report_creator.highcharts import HighchartRender
from report_creator.widgets import RangeField
from report_creator.templatetags.model_report import *
...
class ReportAdmin(object):
    ...
    template_name = 'report_creator/report.html'
    """Template file name to render the report."""
    ...
    def __init__(self, parent_report=None, request=None):
        ...
        if parent_report:
            self.related_inline_field = [f for f, x in [(f, f.model if f.model != self.model else None)
                 for f in self.model._meta.get_fields()
                 if not f.is_relation
                 or f.one_to_one or f.many_to_many
                or (f.many_to_one and f.related_model)] if hasattr(f, 'related_model') and f.related_model is self.parent_report.model][0]

            self.related_inline_accessor = self.related_inline_field.remote_field.get_accessor_name()
            #print(self.related_inline_accessor)
            self.related_fields = ["%s__%s" % (get_model_name(pfield.model), attname) for pfield, attname in self.parent_report.model_fields if not isinstance(pfield,str) and  pfield.model == self.related_inline_field.related_model]
    ...
    def _get_grouper_text(self, groupby_field, value):
        ...
        if value is None or force_text(value) == u'None':#unicode
            if groupby_field is None or groupby_field == u'None':#unicode
            ...
            for k, v in dict(form_fields).items():
                if v is None:
                    ...
                    if '__' in k:
                        ...
                        # for field_lookup in k.split("__")://skh
                            if pre_field:
                    ...
                    if isinstance(model_field, (DateField, DateTimeField)):
                        ...
                        if not hasattr(model_field, 'formfield'):
                            ...
                            # quit()
                            if self.always_show_full_username and (model_field.related_model == User):
    ...
    def get_with_dotvalues(self, resources):
        ...
        for index, model_field in dot_model_fields:
            ...
            # if isinstance(model_field, (unicode,str)) and 'self.' in model_field:#unicode
            if isinstance(model_field,str) and 'self.' in model_field:#unicode

                model_qs = self.model.objects.filter(pk__in=model_ids)
            else:
                model_qs = model_field.related_model.objects.filter(pk__in=model_ids)
